...
MetaWasher는 Python Flask 기반의 사진/동영상 **메타데이터 삭제** 웹 프로그램입니다.  
...
- **삭제 전/후 비교** — 삭제 전후 민감 태그 목록을 시각적으로 비교
...
> ⚠️ **주의**: 이미지 픽셀에 직접 삽입된 위치 정보나 영상 화면/음성에 포함된 개인정보는 메타데이터가 아니므로 삭제되지 않습니다.
...
**Windows**:
- [ExifTool 공식 사이트](https://exiftool.org/)에서 다운로드
...
> **참고**: [Google Cloud Console](https://console.cloud.google.com/)에서 **Maps JavaScript API**와 **Geocoding API**를 활성화해야 합니다. API 키가 없으면 지도 대신 좌표와 Google Maps 링크만 표시됩니다.

## ⚙️ 처리 흐름

```
파일 업로드 → 원본 저장 → 메타데이터 분석 → GPS 위치 추출
     ↓
메타데이터 삭제 (ExifTool 우선, 실패 시 FFmpeg 폴백)
     ↓
삭제 후 메타데이터 재분석 → 전/후 비교 결과 표시
     ↓
개별 파일 또는 ZIP 다운로드 → 1시간 후 자동 삭제
```
...
5. 동영상에서 ExifTool이 실패하면 FFmpeg `-map_metadata -1 -c copy`로 보조 처리합니다.
